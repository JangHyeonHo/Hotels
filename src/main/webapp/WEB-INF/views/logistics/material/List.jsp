...
<!-- 헤더 푸터 건들지 말것(필수는 아님) -->
	...
	<div id = "con">
	<div id = "title">
	<h2><spring:message code="${bound}bound"/> 목록</h2>
	</div>
		...
		<table class = "table table-hover">
			...
			<tr>
				...
				<td>${bat.miPrice }</td>
				<td><fmt:formatDate pattern="yyyy년 MM월 dd일 HH시 mm분" value="${bat.miInbDate }"/></td>
			</tr>
			</c:forEach>
			</c:if>
			...
			<tr>
				...
				<td><fmt:formatDate pattern="yyyy년 MM월 dd일 HH시 mm분" value="${bat.moOutbDate }"/></td>
			</tr>
			</c:forEach>
			</c:if>
	...
	<input type = "button" id = "submitBtn3" onclick="history.back(-1)" value = "뒤로가기" class = "btn-a btn btn-primary btn">
		</div>
	</div>
